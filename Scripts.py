...
def superDigit(n, k):
    ...
    if k == 1:
        return sum_up(super_digit)
    else:
        return sum_up(super_digit * int(k))

def sum_up(n):
    if n < 10:
        return n
    else:
        return sum_up(sum(map(int, list(str(n)))))
...
def insertionSort1(n, arr):
    ...
    while (temp < arr[my_idx]) and (my_idx >= 0):
        arr[my_idx + 1] = arr[my_idx]
